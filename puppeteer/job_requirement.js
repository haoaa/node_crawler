...
const searchKey = '前端'
let hasSentCount = 0
const maxSentCount = 10
const blockKeyWord = ['武汉佰钧成', '中软', '明川', '中软', '平安']
const user = ''
const pass = '';
...
async function sendResume(browser, jobLink) {
  ...
  if (test.toLocaleLowerCase().indexOf('vue') !== -1 && !test.includes('985') && !dispoint) {
    let blocked = blockKeyWord.some(i => company.indexOf(i) !== -1)
    if (!blocked) {
...
async function login(browser, loginSite, user, pass) {
  ...
  await page.waitFor(20000) // 图片识别要手点
}